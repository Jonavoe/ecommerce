...
export const ProductoLista = ({ agregarAlCarrito }) => {
  ...
  useEffect(() => {
    const obtenerTodo = async () => {
      ...
      const productosOther = await obtenerProductos(
        "https://ecommercews.onrender.com/other"
      );
      productosMouses.forEach((producto) => producto.id);
      setProductos([
  ...
  const eliminarProducto = async (id, category) => {
    const url = `https://ecommercews.onrender.com/${category}/${id}`;
    try {
      const response = await fetch(url, {
        method: "DELETE",
      });
    ...
    <div>
      <div className={styles.containerTitle}>
        <h1 className={styles.title}>
          ...
          >
            <option className={styles.option} value="Productos">
              Productos
            </option>
            <option className={styles.option} value="Teclados">
              Teclados
            </option>
